...
INSERT INTO Lecturer(Lecturer_Id,First_name,Last_name,Gender,Email_address,DOB,Designation,Country,City,Street,Ward_no) VALUES (4,'Steve','Vai','Male','steve@jmccol.com',TO_DATE('1988-04-04','YYYY/MM/DD'),'Professor','Bangladesh','Kushtia','Ns road',2);
...
BEGIN
dbms_output.put_line('Welcome to open credit course DBMS');
...
BEGIN
dbms_output.put_line('View on leccturer table');
end;
/
...
BEGIN
dbms_output.put_line('Use of Group By ,Having,order by');
end;
/
select Lecturer_Id, count(Lecturer_Id) from Lecturer_Teaches_Subject group by Lecturer_Id having count(Lecturer_Id)>1 order by Lecturer_Id desc;
...
BEGIN
dbms_output.put_line('Finding third max Total_fees using nested query');
end;
/

select max(Total_fees) from Program where Total_fees <(
select max(Total_fees) from Program where Total_fees <(
select max(Total_fees) from Program));
...
BEGIN
dbms_output.put_line('Use of union operation along with nested query');
end;
/
...
union
select Lecturer_Id from Lecturer where Lecturer_Id in(select Lecturer_Id from Lecturer_Works_For_Department)) order by First_name;


BEGIN
dbms_output.put_line('Use of intersection operation along with nested query');
end;
/
...
BEGIN
dbms_output.put_line('Minus operation');
end;
/
select First_name,Last_name from Lecturer  where Lecturer_Id in(
select Lecturer_Id from Lecturer
minus
select Lecturer_Id from Lecturer where Lecturer_Id in(select Lecturer_Id from Lecturer_Works_For_Department));
...
select d.Roll_no,d.First_name,d.Last_name,l.Contact_number
 from Student d join Student_Contact_number l
  using(Student_Id);


  BEGIN
dbms_output.put_line('use of cross join operation');
end;
/

select d.Roll_no,d.First_name,d.Last_name,l.Contact_number
 from Student d cross join Student_Contact_number l;

 
BEGIN
dbms_output.put_line('use of left outerjoin operation');
end;
/

select d.Roll_no,d.First_name,d.Last_name,l.Contact_number
 from Student d left outer join Student_Contact_number l
  using(Student_Id);


BEGIN
dbms_output.put_line('use of right outerjoin operation');
end;
/

select d.Roll_no,d.First_name,d.Last_name,l.Contact_number
 from Student d right outer join Student_Contact_number l
  using(Student_Id);
  

  
BEGIN
dbms_output.put_line('use of full outerjoin operation');
end;
/
...
BEGIN
dbms_output.put_line('Use of pl sql-Teacher of cse 3100 subject');
end;
/
...
dbms_output.put_line('Use of pl Cursor-All male student from student table');
end;
/
...
BEGIN
dbms_output.put_line('Use of pl sql Procedure-Max Total_fees of a Program');
end;
/
...
DBMS_OUTPUT.PUT_LINE('Max Total_fees of a Program is : '||fee);
END;
/
...
BEGIN
dbms_output.put_line('Use of pl sql Function : Minimum Total_fees of a Program');
end;
/
...
fee Program.Total_fees%type;
BEGIN 
SELECT min(Total_fees) INTO fee FROM Program;
return fee;
END;
/
...
BEGIN
dbms_output.put_line('Use of pl sql TRIGGER :Restriction in insert data on Student_Contact_number');
end;
/



CREATE OR REPLACE TRIGGER sid BEFORE INSERT OR UPDATE ON Student_Contact_number
FOR EACH ROW
DECLARE
i_min constant number := 1;
i_max constant number := 4;
BEGIN
  IF :new.Student_Id > i_max OR :new.Student_Id < i_min THEN
  RAISE_APPLICATION_ERROR(-20000,'New id is too small or large');
END IF;
END;
/

INSERT INTO Student_Contact_number(Student_Id, Contact_number)VALUES(1,'14445333');
 INSERT INTO Student_Contact_number(Student_Id, Contact_number)VALUES(5,'34567333');
select * FROM Student_Contact_number;